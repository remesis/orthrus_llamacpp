...
static int run_orthrus_diff_tensor_tests() {
    llama_quant_model_desc desc = {};
    ...
    const size_t ctx_size = n_names * ggml_tensor_overhead();
    struct ggml_init_params params = { ctx_size, nullptr, true };
    ggml_context_ptr ctx(ggml_init(params));
    ...
    auto check_ftype = [&](llama_ftype ftype) {
        std::vector<ggml_type> result_types(tensors.size());
        llama_quant_compute_types(qs, ftype, tensors.data(), result_types.data(), tensors.size());
        ...
        auto expect_eq = [&](const char * lhs, const char * rhs) {
            if (by_name[lhs] != by_name[rhs]) {
                printf("  FAIL  [%s] %s is %s, expected same as %s (%s)\n",
                        llama_ftype_to_name(ftype), lhs, ggml_type_name(by_name[lhs]), rhs, ggml_type_name(by_name[rhs]));
                ok = false;
            }
        };
        auto expect_ne = [&](const char * lhs, const char * rhs) {
            if (by_name[lhs] == by_name[rhs]) {
                printf("  FAIL  [%s] %s unexpectedly matches %s as %s\n",
                        llama_ftype_to_name(ftype), lhs, rhs, ggml_type_name(by_name[lhs]));
                ok = false;
            }
        };
    ...
    bool ok = true;
    ok &= check_ftype(LLAMA_FTYPE_MOSTLY_Q2_K);
    ok &= check_ftype(LLAMA_FTYPE_MOSTLY_IQ3_XXS);

    printf("  %s  Orthrus diffusion tensor quant category checks\n", ok ? "PASS" : "FAIL");
...
int main(int argc, char ** argv) {
    ...
    const int orthrus_diff_fail = run_orthrus_diff_tensor_tests();
    if (orthrus_diff_fail != 0) {
        return orthrus_diff_fail;
    }
